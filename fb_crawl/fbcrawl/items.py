...
# def parse_date4(date_str_list, lang):
#     if lang == "en":
#         min_str = "mins"
#         hour_str = "hr"
        
#     else:
#         raise Exception(f"Your lang '{lang}' is not current support")
#     format_date = date_str_list[-1].strip()
#     if format_date == min_str:
#         return datetime.today() - timedelta(hours=0, minutes=int(date_str_list[0]))
#     elif format_date == hour_str:
#         return datetime.today() - timedelta(hours=int(date_str_list[0]), minutes=0)
#     else:
#         raise Exception(f"New format datetime {date_str_list}, investigate...")
            ...
            if date[0].isalpha():
                if date[0].lower() == 'ieri':
                    ...
                    month = int(str(datetime.now().date()-timedelta(1)).split(sep='-')[1])
                        ...
                        return datetime(year,month,day)    #return today
...
# l = 3
            ...
            if date[2] == 'ago':
                ...
                day = int(str(datetime.now().date()-timedelta(1)).split(sep='-')[2])
                month = int(str(datetime.now().date()-timedelta(1)).split(sep='-')[1])
                hour, minute = parse_hour_and_mintues(date[-1])
                return datetime(year,month,day, hour=hour, minute=minute).date()
...
def parse_hour_and_mintues(hour_str):
    # 15:50
    hour, minues = list(map(int, hour_str.split(":")))
    return hour, minues
...
def comment_strip(post_id):
    import json
    d = json.loads(post_id[::-1][0]) #nested dict of features
    return str(d['data-commentid'])
...
class CommentsItem(scrapy.Item):
    ...
    last_update = scrapy.Field(      # when was the post published
        output_processor=parse_date3
    )
    text = scrapy.Field(
        output_processor=Join(separator=u'')
    )                       # full text of the post
