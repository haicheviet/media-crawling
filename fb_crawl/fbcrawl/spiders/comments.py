...
from fbcrawl.items import CommentsItem, parse_date, parse_date3
...
class CommentsSpider(FacebookSpider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "post_id" not in kwargs:
            raise AttributeError("You need to specifiy post id default")
        elif "post" in kwargs:
    ...
    def parse_page(self, response):
        ...
        if self.type == "post":
            ...
            for post in response.xpath("//div[contains(@data-ft,'top_level_post_id')]"):
                ...
                last_update = parse_date3([date_string], {"lang": self.lang})
                ...
                if response.meta["flag"] == self.k and self.k >= self.year:
                    ...
                    if new_page:
                        ...
                        while (
                            ...
                            if self.k < self.year:
                                raise CloseSpider(
                                    "Reached last_update: {}. Crawling finished".format(
                                        self.last_update
                                    )
                                )
                            xpath = (
    ...
    def parse_post(self, response):
        ...
        if not response.xpath(path):  # prevents from exec
            ...
            for i, reply in enumerate(response.xpath(path2)):
                ...
                new.add_xpath("last_update", ".//abbr/text()")
                new.add_xpath(
                    "reactions", './/a[contains(@href,"reaction/profile")]//text()'
                )
                new.add_value("url", response.url)
                new.add_value("post_id", self.post_id)
                new.add_xpath("comment_id", './/@id')
                yield new.load_item()
