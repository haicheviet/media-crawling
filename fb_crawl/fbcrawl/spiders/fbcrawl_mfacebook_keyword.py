...
class FacebookSpider(scrapy.Spider):
    ...
    def parse_page(self, response):
        ...
        # select all posts
        for post in response.xpath("//article[contains(@id,'u_0')]/footer"):
            ...
            if current_date is None:
                date_string = post.xpath("./div[1]//abbr/text()").get()
            ...
            new.add_xpath("post_id", "//article[contains(@id,'like')]/@id")
            new.add_xpath("url", "./div[2]/a[4]@href")
            # from scrapy.shell import inspect_response

            # inspect_response(response, self)

            # returns full post-link in a list
            post = post.xpath("./div[2]/a[4]/@href").extract()
        ...
        if self.group == 1:
            ...
            if not new_page:
                ...
                new_page = response.xpath(
                    "//*[@id='see_more_pager']/a").extract()
        else:
            new_page = response.xpath(
                    "//*[@id='see_more_pager']/a/@href").extract()
            # new_page = response.xpath(
        ...
        if not new_page:
            ...
            if "flag" in response.meta:
                self.logger.info(
                    'Page scraped, clicking on "more"! count: {}'.format(
                        self.count
                    )
                )
                yield scrapy.Request(
